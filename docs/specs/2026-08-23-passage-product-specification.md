...
- Do not use the quarantined Book of Mormon base, technically selected corpus, or typed successor for evidence or evaluation. Repaired successor SHA-256 `35ed3713ee222c2778d58f3962c016ea2fef888bc0f8be4edb2b6aacf5641a4d` passed the digest, acquisition-record, fidelity, and maintainer-approval gates and is accepted as inactive corpus `corpus-eb076af14ec6fff84eb40cf0`.
- Activate the exact accepted repaired corpus only through a separate maintainer action. Until then the quarantined old pointer remains operational state, not evidence authority.
