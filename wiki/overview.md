...
- The private control pointer now selects repaired corpus `corpus-eb076af14ec6fff84eb40cf0` with baseline `baseline-27ce2c9404b2a0e0df20859b`. The prior edge-free corpus remains immutable historical control data but is inactive, quarantined, and without editorial authority.
...
- Rebinding the three already approved official-note repairs produced successor SHA-256 `35ed3713ee222c2778d58f3962c016ea2fef888bc0f8be4edb2b6aacf5641a4d`: 9,827 notes, 7,213 reference-bearing notes, 2,614 no-reference notes, zero blockers, and 13,136 edges (7,972 internal and 5,164 external). The maintainer accepted it for the single-maintainer local beta without manual content review after exhaustive automated reconciliation. Corpus `corpus-eb076af14ec6fff84eb40cf0` and baseline `baseline-27ce2c9404b2a0e0df20859b` are now active; HTTP and MCP smoke checks agree on lookup, lexical, and official-reference behavior.
...
- Phase 1 U1-U3 remain completed source-independent groundwork, but U4-U7 are paused. The exact repaired corpus is accepted, verified, and active. The next gate is the locked broad-question product-loop evaluation plus repeated real HTTP/MCP study flows. Citation-resolution and evidence-class failures must be zero, and the maintainer must explicitly judge the local workflow useful enough before hosted-platform work resumes. The maintainer waived off-workstation backup only as a pre-acceptance gate for this local-only beta and accepted rebuild risk; backup and recovery remain required before remote delivery or irreplaceable member data.
...
- Does the active local beta pass its locked evaluation and real-use usefulness gate?
