...
## [2026-08-25] activate | Repaired Book of Mormon corpus activated locally

The maintainer separately authorized activation of the repaired corpus. Passage revalidated the immutable artifact and atomically selected corpus `corpus-eb076af14ec6fff84eb40cf0` with baseline `baseline-27ce2c9404b2a0e0df20859b`. Independent control-store checks confirmed the exact pair, artifact identity, integrity `ok`, and no foreign-key findings; the corrupted historical corpus remains immutable but is now inactive and quarantined.

In-process loopback HTTP and stdio MCP smoke checks agreed on the active identity, corrected 278-character terminal passage, lexical result ordering, and two official edges from the terminal passage. The private approval record now carries the explicit activation authority and result. No persistent server, locked evaluation, remote exposure, hosted deployment, or public submission occurred. The next gate is the locked broad-question evaluation and repeated real local study flow.
