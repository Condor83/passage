...
The v2 grammar and approved reference repairs are active local-beta behavior against the accepted exact corpus. The next gate is the locked broad-question evaluation and repeated real local study flow. Off-workstation backup was explicitly waived only as a pre-acceptance gate for this local-only beta; it remains required before remote delivery or irreplaceable member data.
